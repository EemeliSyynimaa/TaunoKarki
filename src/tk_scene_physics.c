...
void scene_physics_init(void* data)
{
    #if 0
    struct scene_physics_data* state = (struct scene_physics_data*)data;
...
void scene_physics_update(void* data, struct game_input* input, f32 step)
{
    struct scene_physics_data* state = (struct scene_physics_data*)data;
...
void scene_physics_render(void* data)
{
    #if 0
    struct scene_physics_data* state = (struct scene_physics_data*)data;
    struct frame* frame = &state->frames[state->frame_current];
...
struct scene_interface scene_physics_create(struct memory_block* memory)
{
    struct scene_interface result = { 0 };
    result.init = scene_physics_init;
    result.update = scene_physics_update;
    result.render = scene_physics_render;
    result.data = stack_alloc(memory, sizeof(struct scene_physics_data));

    memory_set(result.data, sizeof(struct scene_physics_data), 0);
